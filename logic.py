...
def create_analytics(df):
    ...
    if not stroke_df.empty:
        ...
        axes[2].pie(side_counts, labels=short_labels, autopct='%1.1f%%', colors=['#2196F3', '#00BCD4', '#4CAF50'])
    else: 
...
def predict_stroke(file_path, model_key):
    ...
    if mask_sum > 0:
        confidence_val = np.mean(prob[mask>0]) * 100
    else:
    ...
    area = f"{(mask_sum/(256*256))*100:.2f}%"
    confidence = f"{confidence_val:.1f}%"
    status_ru = "ИНСУЛЬТ" if is_s else "НОРМА"
    
    warning_html = ""
    if is_s and confidence_val < 70:
    ...
    """
    return res_view, input_img_resized, stats_html, "", df_history, pdf_p
...
def process_batch(files, model_key):
    ...
    for i, f in enumerate(files):
        ...
        area_v = (mask_sum/(256*256))*100
        area_str = f"{area_v:.2f}%"
        hu = get_hu_analysis(img, mask, ds) if is_s else "Н/Д"
        raw_side = "Левое" if np.sum(mask[:, :128]) > np.sum(mask[:, 128:]) else "Правое"
        side = get_artery_basin(mask, raw_side) if is_s else "Не выявлено"
        confidence = f"{confidence_val:.1f}%"
    ...
    """
    return create_analytics(df_ana), stats_html, None, df_display, pdf_p, df_display
